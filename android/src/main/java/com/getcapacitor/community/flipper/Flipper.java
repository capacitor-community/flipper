...
public class Flipper extends Plugin {
    ...
    private final static String TAG = "Flipper";
    ...
    public void load() {
        ...
        if (this.applicationContext == null) {
            this.applicationContext = this.bridge.getActivity().getApplicationContext();
        }
    }
    ...
    public void initialize(PluginCall call) {
        ...
        if (this.isEnabled) {
            ...
            final FlipperClient client = AndroidFlipperClient.getInstance(this.applicationContext);

            if (this.layoutInspectorEnabled) {
                client.addPlugin(new InspectorFlipperPlugin(this.applicationContext, DescriptorMapping.withDefaults()));
            }
            ...
            if (this.databaseEnabled) {
                if (!this.databasePath.isEmpty()) {
                    client.addPlugin(new DatabasesFlipperPlugin(new SqliteDatabaseDriver(this.applicationContext, new SqliteDatabaseProvider() {
                        @Override
                        public List<File> getDatabaseFiles() {
                            List<File> databaseFiles = new ArrayList<>();
                            for (String databaseName : applicationContext.databaseList()) {
                                databaseFiles.add(applicationContext.getDatabasePath(databaseName));
                            }
    ...
    public void emulateCrash(PluginCall call) {
        if (this.crashReportEnabled && call.hasOption("message")) {
            ...
            CrashReporterPlugin.getInstance().sendExceptionMessage(Thread.currentThread(), new Throwable(message));
